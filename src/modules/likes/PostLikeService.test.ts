...
describe("PostLikeService", () => {
  ...
  describe("addLikePost", () => {
    it("debe agregar un like exitosamente", async () => {
      ...
      vi.mocked(mockConn.execute).mockImplementation(
        (sql: any, _params?: any) => {
          ...
          if (sqlStr.includes("select id from post_likes")) {
      ...
      const result = await postLikeService.addLikePost(1, 5);
    ...
    it("debe lanzar error si ya existe el like", async () => {
      ...
      await expect(postLikeService.addLikePost(1, 5)).rejects.toThrow(
        "Ya te gusta esta publicación",
      );
    });
  ...
  describe("removeLikePost", () => {
    it("debe eliminar un like exitosamente", async () => {
      // Mock para getConnection y su ejecución dentro de removeLikePost
      const mockConn = await pool.getConnection();
      vi.mocked(mockConn.execute).mockImplementation((sql: any) => {
        const sqlStr = typeof sql === "string" ? sql.toLowerCase() : "";
        ...
        if (sqlStr.includes("select")) {
          return Promise.resolve([[{ user_id: 2 }], {}] as any);
        }
        return Promise.resolve([{}, {}] as any);
      });
      const result = await postLikeService.removeLikePost(1, 5);
  ...
  describe("getLikeCount", () => {
    it("debe retornar la cantidad de likes", async () => {
      ...
      const result = await postLikeService.getLikeCount(1);
    ...
    it("debe retornar 0 si no hay likes", async () => {
      ...
      const result = await postLikeService.getLikeCount(999);
  ...
  describe("hasLiked", () => {
    it("debe retornar true si ya dio like", async () => {
      ...
      const result = await postLikeService.hasLiked(1, 5);
